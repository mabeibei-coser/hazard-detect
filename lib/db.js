...
// 读单条报告（含 hazards JSON + 现场照片）。校验归属：只返回这个手机号自己的报告。
// 用于台账下载链路（前端拿 reportId + token，后端按归属取 report 生成 Excel）。
export function getReportById(reportId, userPhone) {
  const db = getDb();
  const row = db
    .prepare(
      `SELECT id, user_id, user_phone, created_at, scenario, scenario_label,
              hazard_count, report_json, image_base64, image_mime
       FROM reports
       WHERE id = ? AND user_phone = ?`
    )
    .get(reportId, userPhone);
  if (!row) return null;
  let hazards = [];
  try {
    hazards = JSON.parse(row.report_json);
    if (!Array.isArray(hazards)) hazards = [hazards];
  } catch {
    hazards = [];
  }
  return {
